...
WHERE (Position = 'EDGE' OR Position = 'DE' OR Position = 'LB');
...
SELECT Player_Name, 
	...
	(CASE
		WHEN [Fourty_sec] < 4.45 THEN 10
		WHEN [Fourty_sec] < 4.5 AND [Fourty_sec] >= 4.45 THEN 9.5
		WHEN [Fourty_sec] < 4.55 AND [Fourty_sec] >= 4.5 THEN 9.0
		WHEN [Fourty_sec] < 4.6 AND [Fourty_sec] >= 4.55 THEN 8.5
		WHEN [Fourty_sec] < 4.65 AND [Fourty_sec] >= 4.6 THEN 8.0
		WHEN [Fourty_sec] < 4.7 AND [Fourty_sec] >= 4.65 THEN 7.0
		WHEN [Fourty_sec] < 4.8 AND [Fourty_sec] >= 4.7 THEN 6.0
		WHEN [Fourty_sec] < 4.9 AND [Fourty_sec] >= 4.8 THEN 5.0

		WHEN [Fourty_sec] IS NULL THEN NULL
		
		ELSE 4.0

	END) AS LB_FourtyGrade,
	...
	END) AS LB_BroadJumpGrade

	INTO LineBackerGrades
...
ALTER TABLE dbo.LineBackerGrades
...
SELECT * FROM dbo.LineBackerGrades
	WHERE (LB_BroadJumpGrade IS NULL)
		AND (LB_FourtyGrade IS NULL)
		AND (LB_VerticalGrade IS NULL)

DELETE FROM dbo.LineBackerGrades
	WHERE (LB_BroadJumpGrade IS NULL)
		AND (LB_FourtyGrade IS NULL)
		AND (LB_VerticalGrade IS NULL)
...
UPDATE dbo.LineBackerGrades
SET AthleticGrade = (LB_FourtyGrade + LB_BroadJumpGrade + LB_VerticalGrade) / 3

UPDATE dbo.LineBackerGrades
SET AthleticGrade = (LB_VerticalGrade + LB_BroadJumpGrade) / 2
WHERE LB_FourtyGrade IS NULL

UPDATE dbo.LineBackerGrades
SET AthleticGrade = (LB_FourtyGrade + LB_BroadJumpGrade) / 2
WHERE LB_VerticalGrade IS NULL

UPDATE dbo.LineBackerGrades
SET AthleticGrade = (LB_FourtyGrade + LB_VerticalGrade) / 2
WHERE LB_BroadJumpGrade IS NULL

UPDATE dbo.LineBackerGrades
SET AthleticGrade = LB_FourtyGrade
WHERE LB_VerticalGrade IS NULL AND LB_BroadJumpGrade IS NULL

UPDATE dbo.LineBackerGrades
SET AthleticGrade = LB_VerticalGrade
WHERE LB_FourtyGrade IS NULL AND LB_BroadJumpGrade IS NULL

UPDATE dbo.LineBackerGrades
SET AthleticGrade = LB_BroadJumpGrade
WHERE LB_FourtyGrade IS NULL AND LB_VerticalGrade IS NULL
